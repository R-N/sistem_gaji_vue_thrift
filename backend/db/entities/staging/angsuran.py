from sqlalchemy.orm import reconstructor
from sqlalchemy.ext.hybrid import hybrid_property

from .base import DbStagingEntity
from ..mixin import MxAngsuran, MxStaging
from .karyawan import DbKaryawan
from .jenis_angsuran import DbJenisAngsuran


class DbAngsuran(MxStaging, MxAngsuran, DbStagingEntity):

    @hybrid_property
    def real_enabled(self):
        return self.enabled and self.karyawan.real_enabled and self.jenis_angsuran.real_enabled

    @real_enabled.expression
    def real_enabled(cls):
        return cls.enabled & DbKaryawan.real_enabled & DbJenisAngsuran.real_enabled
