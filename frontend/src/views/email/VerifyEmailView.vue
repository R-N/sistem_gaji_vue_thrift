...
import VerifyEmailForm from '@/components/email/verify/VerifyEmailForm';
import ChangeEmailForm from '@/components/email/verify/ChangeEmailForm';
...
class VerifyEmailView extends BaseView {
	...
	async checkHasPassword(){
		...
		view.busy = true;
		try{
			view.hasPassword = await stores.client.email.has_password(view.token);
		} catch (error) {
