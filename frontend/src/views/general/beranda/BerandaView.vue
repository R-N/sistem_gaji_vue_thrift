...
class BerandaView extends BaseView {
	...
	async hello(){
		...
		view.busy=true;
		try{
			this.msg = await clientStore.hello.hello.hello_admin_utama();
		} catch (error){
	...
	async createBackup(){
		...
		try{
			...
			await clientStore.system.backup.create_backup(this.backupName);
	...
	async deleteBackup(){
		...
		try{
			if (!this.backupName) throw new TFileError({ code: TFileErrorCode.FILE_NAME_EMPTY});
			await clientStore.system.backup.delete_backup(this.backupName);
	...
	async fetchBackups(){
		...
		view.busy=true;
		try{
			var backups = await clientStore.system.backup.fetch_backups();
	...
	async downloadBackup(){
		...
		view.busy=true;
		try{
			let blob = await clientStore.system.backup.download_backup(this.backupName);
	...
	async uploadBackup(){
		...
		view.busy=true;
		try{
			let uploadedName = await clientStore.system.backup.upload_backup(this.file, this.backupName);
			this.file = null;
